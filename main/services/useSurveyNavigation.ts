...
export const useSurveyNavigation = () => {
  ...
  const checkHasSurveysAndGetSurveys = (currentRoute) => {
    return (
      surveysEnum.find((survey) => {
        ...
        const currentFullPathDecoded = decodeURIComponent(currentRoute.fullPath || '')
  ...
  const getFormIdFromArray = async (name, list) => {
    ...
    const countField = name === 'trainings' ? 'totalTrainingsCount' : 'totalExamsCount'
    const count = subscriptionsStore.state.userCurrentSubVal?subscriptionsStore.state.userCurrentSubVal[countField]:0
  ...
  const alreadySubmitted = (formId) => {
    const completedIds = JSON.parse(localStorage.getItem('completedSurveyIds') || '[]')
