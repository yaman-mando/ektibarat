  ...
  <user-panel-wrapper>
    <div class="mx-auto">
      ...
      <div class="flex flex-col xl1200:flex-row gap-[20px]">
        ...
        <div
          class="flex flex-1/3 flex-col gap-y-[20px] w-full 2xl:min-w-[340px]"
        >
          <!-- rate square -->
          <div
            class="h-[160px] bg-white shadow-custom rounded-[8px] p-[15px] grid justify-items-center relative"
          >
              ...
              >
                <g clip-path="url(#clip0_608_900)">
                  <rect
                    ...
                    height="12.72"
                    rx="0.96"
                    fill="url(#paint0_linear_608_900)"
                  />
                  <rect
                    x="11.5199"
                    y="15.6557"
            ...
            <span
              class="text-center text-blue-d6 h-[20px] text-[18px] 2xl:text-[22px] font-bold"
            >
              الدرجة المتوقعة
            </span>

            <span
              class="text-center text-dark-63 h-[16px] text-[14px] 2xl:text-[16px] font-medium"
            >
              في الاختبار الحقيقي
            </span>
            ...
            <div class="relative mt-auto">
              <div class="text-center text-green-8c font-bold leading-none">
                <span
                  v-if="false"
                  class="text-[40px] 2xl:text-[50px]"
                >
                  95~98
                </span>
                <span
                  v-if="true"
                  class="text-[22px] 2xl:text-[26px]"
                >
              ...
              <div class="flex items-center justify-around">
                <div class="flex items-center gap-x-[12px]">
                  ...
                  <span
                    class="font-bold text-dark-2b text-[10px] 2xl:text-[12px]"
                  >
                    74
                  </span>
                </div>
          ...
          <!-- palne square -->
          <div
            class="min-h-[300px] bg-white shadow-custom rounded-[8px] p-[20px_15px] grid"
          >
            <!-- First line -->
            <div class="flex items-center justify-between h-[50px]">
              ...
              <div class="flex flex-col items-center">
                ...
                <span
                  class="text-gray-8f text-[10px] 2xl:text-[12px] font-medium"
                >
                  {{ formatTime(planGrades.timeDone) }} |
                  {{ formatTime(planGrades.timeRequired) }}
                </span>
              </div>
            </div>
            ...
            <div class="mt-[15px] h-[84px] grid items-center relative">
              ...
              >
                ...
                <div
                  ...
                  :style="{ width: `${planGrades.percentageDone}%` }"
                ></div>
                ...
                <div
                  ...
                  :style="{
                    width: `${planGrades.percentageRequired - planGrades.percentageDone}%`,
                    right: `${planGrades.percentageDone}%`,
                  }"
                ></div>
              </div>
              ...
              <div
                :style="{ right: `calc(${planGrades.percentageDone}% - 30px)` }"
                class="absolute -top-[10px] h-[27px] grid gap-y-[5px] justify-items-center w-[40px]"
              >
                <div class="text-[10px] 2xl:text-[12px] text-black font-medium">
                  أنت هنا
                </div>
                <svg
                  width="16"
              ...
              <div
                :style="{
                  right: `calc(${planGrades.percentageRequired}% - 45px)`,
                }"
            ...
            <!-- Last week / month -->
            <div
              class="flex flex-wrap gap-y-[10px] justify-around items-center mt-[28px] h-[30px]"
            >
              <div
              ...
              >
                <div class="font-bold text-[24px] 2xl:text-[28px] text-dark-63">
                  {{ planGrades.lastMonth.percentage }}%
                </div>
                <div
                ...
                >
                  <span>آخر الشهر</span>
                  <span>
                    {{ formatTime(planGrades.lastMonth.timeDone) }} |
                    {{ formatTime(planGrades.lastMonth.timeRequired) }}
                  </span>
                </div>
              </div>
              <div
              ...
              >
                <div class="font-bold text-[24px] 2xl:text-[28px] text-dark-63">
                  {{ planGrades.lastWeek.percentage }}%
                </div>
                <div
                  class="flex flex-col items-center text-[12px] 2xl:text-[14px] font-medium text-gray-8f"
                >
                  <span>آخر أسبوع</span>
                  <span>
                    {{ formatTime(planGrades.lastWeek.timeDone) }} |
                    {{ formatTime(planGrades.lastWeek.timeRequired) }}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div class="flex items-center justify-around h-[8px]">
              <div class="flex items-center gap-x-[12px]">
                ...
                <span class="text-dark-2b text-[10px] 2xl:text-[12px]">
                  {{ planGrades.startTraining }}
                </span>
              </div>
              <div class="flex items-center gap-x-[12px]">
                <span
        ...
        <div
          class="w-full flex-2/3 2xl:min-w-[700px] h-[480px] bg-white shadow-md rounded-lg p-4 relative"
        >
          <!-- for unsubscribe user -->
          <div
            v-if="false"
          ...
          >
            <component
              ...
              v-if="apexChartService.apexComponent.value"
              :key="chartKey"
              type="area"
              :height="'280px'"
          ...
          >
            <div
              class="text-[14px] text-gray-8f font-medium space-x-[5px] lg:space-x-[20px]"
            >
              <span>الأسئلة</span>
      ...
      <!-- tables filter -->
      <div
        class="flex flex-col md:flex-row md:items-center md:justify-baseline mt-[25px] gap-x-[30px] gap-y-[15px]"
      >
        <h2 class="text-[24px] font-bold text-blue-d6 text-right">
          تفاصيل التدريب
        </h2>
            ...
            <div class="space-y-2">
              ...
              >
                ...
                >
                  ...
                  <div class="w-[20%] text-gray-63 font-medium text-center">
                    {{ formatTime(child.trainingTime) }} ساعة
                  </div>
                </div>
                ...
                >
                  <div
                    class="w-[55%] h-[100%] flex justify-center items-center"
                  >
                    <template v-if="child.isEnabled">
                      <span
                        v-if="child.rate === 0"
                        class="text-[14px] font-bold text-purple-e0"
                      >
                        عدد الأسئلة غير كاف
                      </span>
                      <rate-progress-bar
                  ...
                  <div
                    class="w-[45%] h-[100%] flex justify-center items-center border-r-1 border-[#BCCCDB]"
                  >
                    <button
...
const items = trianingDeltailsData
  ...
  .map((parent) => {
    const children = trianingDeltailsData.filter(
      (child) => child.parentId === parent.categoryId
    );
    return {
      ...parent,
      details: children,
...
onMounted(() => {
  if (backendDays <= 10)
    selectedPeriod.value = 0; // daily
  else if (backendDays < 90)
    selectedPeriod.value = 1; // weekly
