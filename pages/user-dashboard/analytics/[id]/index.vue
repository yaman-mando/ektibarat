<template>
  <user-panel-wrapper
    ...
    :pageName="`تحليل قسم: ${mainCategory?.categoryName}`"
    :withBackPage="true"
  ...
  >
    ...
    <div>
      ...
      <div class="flex gap-[20px] flex-col md:flex-row">
        ...
        >
          <app-data-wrapper
            :loading="userPanelStore.fetching.analyticsDetails"
            :data="mainCategory"
            loadingType="text"
          ...
          >
            <div class="flex items-center justify-between w-full">
              <div class="grid">
                <span
                  class="text-[20px] xl:text-[24px] text-blue-d6 dark:text-white font-bold"
                >
                  تقييمك
                </span>
                <div
                  class="text-[14px] xl:text-[16px] font-medium flex items-center"
                  :class="`text-${getRateColor(mainCategory?.rate)}`"
                >
                  <div
                    class="w-[8px] h-[8px] rounded-full ml-[5px]"
                    :class="`bg-${getRateColor(mainCategory?.rate)}`"
                  ></div>
              ...
              <div class="relative w-[70px] xl:w-[80px] h-[70px] xl:h-[80px]">
                ...
                >
                  ...
                  <path
                    ...
                    :stroke-dasharray="`${mainCategory?.rate}, 100`"
            ...
            >
              <div class="flex-1 space-y-[4px]">
                <div
                  class="text-purple-78 text-[15px] xl:text-[16px] font-bold"
                >
                  الأسئلة
                </div>
                <div>
                  {{ dateFormat.formatNoData(mainCategory?.questionsCount) }}
                </div>
              </div>
              <div class="flex-1 space-y-[4px]">
                <div
              ...
              <div class="flex-1 space-y-[4px]">
                ...
                <div>
                  {{
                    dateFormat.formatStoSSMM_MMHHWithText(
                      mainCategory?.totalTime
                    )
                  }}
                </div>
              </div>
            ...
            <p
              class="text-[13px] xl:text-[14px] font-medium text-center text-gray-63 dark:text-gray-400"
            >
              استمر في التدريب لترتفع مستواك في هذه المهارة
            </p>

            <div
              class="grid gap-[8px] xl:gap-[10px] mt-[10px] justify-items-center w-full"
            >
              <button
              ...
              >
                <img
                  src="/images/svg/user-panel/lamp-white.svg"
                  class="w-[18px] xl:w-[20px]"
                />
                نصائح اختبارات
              </button>
              <button
              ...
              >
                <img
                  src="/images/svg/user-panel/dumbbell.svg"
                  class="w-[18px] xl:w-[20px]"
                />
                تدرب على القسم
              </button>
        ...
        >
          ...
          <!-- الرسم البياني -->
          <app-data-wrapper
            :loading="userPanelStore.fetching.analyzeDetailsChartForStudent"
            :data="chartData"
            loadingType="text"
            emptyText="لا توجد بيانات متاحة"
          >
            <div
              v-if="chartData.some((k) => k.count > 0)"
      ...
      <!-- row-2 -->
      <div
        v-if="tableCategories && tableCategories.length > 0"
            ...
            >
              <div class="max-w-[100vw] overflow-x-auto custom-scroll">
                <div class="space-y-2 min-w-[1020px]">
                  ...
                  <div
                    v-for="(child, i) in tableCategories"
                    :key="i"
                  ...
                  >
                    ...
                    >
                      ...
                      <div class="w-[15%] text-center">
                        ...
                        <span class="text-red-5e font-bold text-[14px]">
                          {{
                            dateFormat.secondsToMMSS(
                              child.allStudentsTimeTakenRate
                            )
                          }}
                        </span>
                      </div>
                      ...
                      <div class="w-[15%] text-center">
                        <app-g-progress-bar
                          v-if="child.rate > 0"
                          radius="4px"
                          :bgClass="`bg-${getRateColor(child.rate)}`"
                          :hasShadow="true"
                          height="18px"
                          :showText="true"
                          :value="child.rate"
                    ...
                    <div
                      class="flex flex-[30%] h-[100%] items-center rounded-l-[8px] border border-[#BCCCDB]"
                      :class="i % 2 === 0 ? 'bg-white' : 'bg-[#F5F7FA]'"
                    >
                      <div
                        ...
                        <template v-else>
                          ...
                          <template v-if="child.hasNoMasteredSkills">
                            <button
                              ...
                              @click="openSkillsModal(child.categoryId)"
                            >
                              للحفظ
                            </button>
                          </template>
    ...
    <app-skills-modal
      v-model="isOpenSkillsModal"
      :skillId="selectedSkillId"
    />
    <advice-loading v-if="isLoadingAdvices" />
    <advice-modal
      v-if="showAdviceModal"
      :content="advicesModalData"
      @close="showAdviceModal = false"
    />
  </user-panel-wrapper>
</template>
...
function toAnalytics() {
  if (
    userData.value.role !== UserRoles.schoolManager &&
    userData.value.role !== UserRoles.teacher
  ) {
...
const mainData = computed(() => {
  return isStudent.value
    ? userPanelStore.analyticsDetails
    : userPanelStore.analyticsDetailsForTeacher?.analyzes;
});

const mainCategory = computed(() => {
  return mainData.value?.find((k) => k.categoryId === Number(catId));
});

const tableCategories = computed(() => {
  return mainData.value?.filter((k) => k.categoryId !== Number(catId));
});
...
onMounted(async () => {
  ...
  if (catId) {
    if (isStudent.value) {
      await userPanelStore.getAnalyticsDetails(catId);
    } else {
      await userPanelStore.getAnalyticsDetailsForTeacher(catId, stdId);
    }
