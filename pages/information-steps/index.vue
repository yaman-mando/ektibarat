<template>
  <div
    class="min-h-screen bg-gray-fb py-[20px]"
    style="direction: rtl"
  >
    ...
    <div
      class="w-[1060px] max-w-[calc(100vw-30px)] m-auto bg-white rounded-[8px] pb-[50px] min-h-[530px]"
      :class="{ grid: step !== 3 }"
      style="box-shadow: 0px 0px 10px 0px #00000026"
    >
      <!-- Progress Bar & Back Arrow -->
      <div
        class="flex items-center gap-[6px] h-[44px] p-[0_15px] border-b-[1px] border-b-[#BCCCDB]"
      >
        <button
          :disabled="step < 2"
        ...
        >
          <div
            class="h-[12px] rounded transition-all duration-500"
      ...
      <div
        class="my-auto"
        :class="{ '!mt-[60px]': step === 3 }"
      >
        <!-- Step 1 -->
        <div v-if="step === 1">
          <h2
            class="text-[30px] sm:text-[40px] font-bold !text-center text-blue-d6"
          >
            خلنا نتعرّف عليك
          </h2>
          ...
          <div class="mt-[30px]">
            <p class="!text-center font-medium text-blue-d6 text-[20px]">
              اختر شخصيّتك
            </p>
        ...
        <div
          v-if="step === 2"
          class="space-y-[30px] w-[300px] grid justify-self-center"
        >
          <client-only>
            <div class="space-y-[10px]">
              <label class="block text-[20px] font-medium text-dark-2b">
                مدينتك في السعودية
              </label>
              <v-select
                v-model="form.cityId"
        ...
        <div
          v-if="step === 3"
          class="grid gap-y-[50px] justify-center"
        >
          <h2
            class="text-blue-d6 text-[30px] sm:text-[40px] font-bold !text-center mb-0"
          >
            في أي صف تدرس؟
          </h2>
          <div class="space-y-[20px] grid justify-center">
            <button
              ...
              :class="[
                {
                  'border !border-blue-d6': form.grades === grade.id,
                  'border !border-red-500':
                    errors.grades && form.grades !== grade.id,
                },
                'border border-[#E5E5E5] w-[300px] h-[50px] bg-gray-fa rounded-[10px] text-dark-2b text-[20px] font-medium cursor-pointer',
              ]"
        ...
        <div
          v-if="step === 4"
          class="grid grid-rows-2 gap-y-[30px] justify-center"
        >
          <div class="space-y-[10px] grid">
            <label class="text-dark-2b text-[20px] font-medium">
              أي محاولة قدرات هذي لك؟
            </label>
            <div class="flex gap-[12px]">
              <div
                ...
                :class="[
                  form.qodoratAttempt === n
                    ? 'bg-[#F8F3FF] border-1 border-purple-e0 text-purple-e0'
                    : 'bg-[#F5F7FA] border-1 border-[#E5E5E5] text-dark-2b',
                ]"
                @click="
                  form.qodoratAttempt = n;
                  form.lastQodoratMark = null;
                "
              >
                {{ n }}
              </div>
          ...
          <div
            v-show="form.qodoratAttempt && form.qodoratAttempt > 1"
            class="space-y-[10px] grid justify-center"
          >
            <label class="text-dark-2b text-[20px] font-medium">
              كم آخر درجة قدرات جبتها؟
            </label>
            <div class="relative w-[300px]">
              <input
                v-model.number="form.lastQodoratMark"
                type="number"
                min="1"
                max="100"
                :class="[
        ...
        >
          <div class="w-[300px] flex justify-self-center relative mt-[100px]">
            <div
              class="w-[220px] h-[90px] flex justify-center absolute right-0 sm:right-[-70px] top-[-60px] items-center text-[18px] font-medium text-dark-63"
              style="box-shadow: 0px 0px 10px 0px #00000026"
            >
              أنت الحين جاهز
              <br />
            ...
            <img
              src="/images/robot-info.png"
              alt="robot"
              class="sm:mx-auto mr-auto ml-0 w-[160px] scale-x-[-1]"
            />
          </div>
        </div>
      </div>
      ...
      <button
        ...
        @click="step === 5 ? startTraining() : toStep()"
      >
...
const form = ref({
  firstName: '',
  lastName: '',
  sex: null as null | any,
  cityId: null as null | any,
  schoolId: null as null | any,
  grades: null as null | any,
  qodoratAttempt: null as null | any,
  lastQodoratMark: null as null | any,
});
  ...
  switch (step.value) {
    ...
    case 2:
      if (!form.value.cityId) {
        // eslint-disable-next-line vue/no-side-effects-in-computed-properties
        errors.value.cityId = true;
        valid = false;
      }
      if (!form.value.schoolId) {
        // eslint-disable-next-line vue/no-side-effects-in-computed-properties
        errors.value.schoolId = true;
        valid = false;
      }
      break;
    case 3:
      if (!form.value.grades) {
        // eslint-disable-next-line vue/no-side-effects-in-computed-properties
        errors.value.grades = true;
        valid = false;
      }
      break;
    case 4:
      if (!form.value.qodoratAttempt) {
        // eslint-disable-next-line vue/no-side-effects-in-computed-properties
        errors.value.qodoratAttempt = true;
        valid = false;
      }
      if (
        form.value.qodoratAttempt > 1 &&
        (!form.value.lastQodoratMark || form.value.lastQodoratMark < 1)
      ) {
        // eslint-disable-next-line vue/no-side-effects-in-computed-properties
...
const sendForm = async () => {
  try {
    ...
    const { data: res } = await $axios.put(`/profile`, form.value);
    if (res) {
