    ...
    <div class="u-wrapper">
        ...
        <template v-if="activeSection === editSections.name">
            <vee-form v-slot="{ handleSubmit }" class="ac-wrapper name m-auto">
                ...
                <div class="w-[300px] m-auto grid gap-y-[20px] mt-[25px]">
                    ...
                    <div class="m-note">
                        <span>
                            أدخل اسمك الصحيح باللغة العربية أو الإنجليزية
                        </span>
            ...
            <div class="r-wrapper">
                <div class="r-element">
                    <div class="r-1">
                        ...
                        <span v-if="appAuth.state.userData?.firstName || appAuth.state.userData?.lastName"
                            class="r-val">
                            {{
                    ...
                    <div class="r-action" @click="setActiveSection(editSections.name)">
                        <span v-if="appAuth.state.userData?.firstName || appAuth.state.userData?.lastName" class="edit">
                            تعديل
                        </span>
        ...
        <template v-if="activeSection === editSections.phone">
            <vee-form v-slot="{ handleSubmit, meta }" class="ac-wrapper name m-auto">
                ...
                <div class="w-[300px] m-auto grid gap-y-[20px] mt-[25px]">
                    ...
                    <div class="m-note">
                        <span>
                            سنرسل رمز التحقق إلى رقمك الجديد على واتساب
                        </span>
                        <span>
                            ولن يتم تغيير رقمك إلا بعد إدخال الرمز
                        </span>
                    </div>
            ...
            <div class="r-wrapper">
                <div class="r-element">
                    <div class="r-1">
                        ...
                        <span v-if="appAuth.state.userData?.phoneNumber" class="r-val direction-ltr"
                            style="direction: ltr">
        ...
        <template v-if="activeSection === editSections.email">
            <vee-form v-slot="{ handleSubmit, meta: formMeta }" class="ac-wrapper name m-auto">
                ...
                <label class=" text-dark-2b font-bold text-[16px]">البريد الإلكتروني</label>
                <div class="w-[300px] m-auto grid gap-y-[20px] mt-[25px]">
                    <div class="ac-control-wrapper m-0">
                    ...
                    <div class="m-note">
                        <span>
                            سنرسل رمز التحقق إلى بريدك الإلكتروني الجديد
                        </span>
                        <span>
                            ولن يتم تغيير البريد إلا بعد إدخال الرمز
                        </span>
                    </div>
                    <div class="ac-action">
                        <app-overlay v-if="processing" />
        ...
        <template v-if="activeSection === editSections.school">
            <vee-form v-slot="{ handleSubmit }" class="ac-wrapper name m-auto">
                ...
                <div class="w-[300px] m-auto grid gap-y-[20px] mt-[25px]">
                    ...
                    <div class="grid gap-y-[10px] ">
                        <label>المدرسة</label>
                        <v-select v-model="profileInfo.schoolId" :options="schools" :reduce="school => school.id"
                            label="label" :clearable="false" placeholder="اختر مدرسة" :disabled="!profileInfo.cityId"
                            style="direction: rtl" :class="[
                                'w-full',
                                'bg-white',
                                !profileInfo.schoolId ? 'border border-red-500 rounded' : '',
                                'text-right',
                                'rtl',
                            ]" dir="rtl" />
                    </div>
            ...
            <div class="r-wrapper">
                <div class="r-element">
                    <div class="r-1">
                        ...
                        <span v-if="appAuth.state.userData?.schoolId || appAuth.state.userData?.cityId" class="r-val">
                            {{
                    ...
                    <div class="r-action" @click="setActiveSection(editSections.school)">
                        <span v-if="appAuth.state.userData?.schoolId || appAuth.state.userData?.cityId" class="edit">
                            تعديل
                        </span>
                        <span v-else class="add">
                            إضافة
            ...
            <vee-form v-slot="{ handleSubmit }" class="ac-wrapper name m-auto">
                ...
                <div class="w-[300px] m-auto grid gap-y-[20px] mt-[25px]">
                    <div class="grid gap-y-[10px]">
                        ...
                        <v-select v-model="profileInfo.grades" :options="grades" :reduce="grade => grade.id"
                            label="label" :clearable="false" placeholder="اختر الصف" :class="[
                                'w-full',
                                'bg-white',
                    ...
                    <div class="ac-action">
                        ...
                        <button :disabled="!profileInfo.grades" class="btn save"
                            @click.prevent="handleSubmit(sendForm)">
                            <span>حفظ</span>
                        </button>
                    </div>
        ...
        <lazy-prime-dialog id="modal-edit-mail" v-model:visible="openMailModal" :closable="true" :closeOnEscape="true"
            ...
            <div :class="{ 'wrong-code': wrongCode }" class="code-part">
                ...
                <div class="w-full relative">
                    <app-overlay v-if="processing" />
                    <code-input :fieldHeight="45" :fieldWidth="45" :fields="4" :radius="8" :required="true"
...
import { dataURLtoFile, processText, toBase64 } from '~/main/utils/shared-utils';
...
const callApiChangeWhatsApp = async () => {
    ...
    try {
        const { data: result } = await $axios.post('/identity/requestChangeWhatsApp', payload);
        if (result === true) {
            checkWhatsAppError.value = false;
            await appAuth.fetchUser();
            profileInfo.whatsAppNumber = oldwhatsAppNumber.value = appAuth.state.userData?.whatsAppNumber;
...
const sendMail = async () => {
    ...
    try {
        const { data: response } = await $axios.post('/identity/requestChangeEmail', {
...
const sendCode = async (code: string) => {
    try {
        processing.value = true;
        const codeForm = {
            method: openMailModal.value ? requestData.value?.method : requestDataPhone.value.method,
            code,
            userId: openMailModal.value ? requestData.value?.id : requestDataPhone.value.id,
        };

        const res = await $axios.post(
            openMailModal.value ? '/identity/confirmChangeEmail' : '/identity/confirmChangePhone',
            codeForm
        );
...
const isMailConfirmed = () => {
    return (
        profileInfo.emailConfirmed && profileInfo.email === appAuth.state.userData?.email
    );
};
...
const togglePassword1 = () =>
    (password1Type.value = password1Type.value === 'password' ? 'text' : 'password');
const togglePassword2 = () =>
    (password2Type.value = password2Type.value === 'password' ? 'text' : 'password');
const togglePassword3 = () =>
    (password3Type.value = password3Type.value === 'password' ? 'text' : 'password');
