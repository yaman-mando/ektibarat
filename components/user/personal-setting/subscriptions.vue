    ...
    <template v-else>
      <lazy-vee-validate-provider>
        <template v-if="!successPaymentCase">
          <div v-if="windowSize.isDesktop" class="rw-1 !mis-0" :class="{ 'is-sub': activeStep == steps.subscriptions }">
            <span>الاشتراكات</span>
          </div>
          <template v-if="activeStep == steps.subscriptions">
            <b-button @click="goToSubscriptionsParent()">باقات</b-button>
...
:class="{
                          ...
                          <div class="c-content">
                            <img src="/images/svg/Mada.svg" alt="اختبارات" />
                            <img src="/images/svg/Visa.svg" alt="اختبارات" />
                            <img src="/images/svg/Mastercard.svg" alt="اختبارات" />
                          </div>
                        </div>
                        <app-overlay v-if="!appleIsLoaded" />
...
class="caption red" style="
        ...
        <template v-else>
          <div class="confirm-payment">
            ...
            <span v-if="!isFreePayment" class="c-mail">
              أرسلنا نسخة من إيصال الدفع إلى بريدك الإلكتروني
              <a :href="'mailto:' + appAuth.user.email">
                {{ appAuth.user.email }}
              </a>
            </span>
            <app-button class="!mt-3" label="لوحتي التعليمية" @click="toLearningPanel()" />
          </div>
        </template>
      </lazy-vee-validate-provider>
    </template>
