<template>
    <div v-if="jsonData" class="contact-us">
        <div class="contents">
            ...
            <div class="cl-end">
                ...
                <lazy-vee-validate-provider>
                    <div class="contact-form">
                        <vee-form ref="form_ref" v-slot="{ handleSubmit }"
                            class="form-el w-full flex flex-col items-start justify-start">
                            <div class="form-rw-1">
                                <form-input v-model:inputValue="form.name" inputId="fullName" :isDisabled="authStore.state.isLoggedIn &&
                                    !!authStore.state.userData?.firstName &&
                                    !!authStore.state.userData.lastName
                                    " :rules="{ required: true }" inputPlaceholder="الاسم" />
                                <form-input v-model:inputValue="form.email" inputId="email" :isDisabled="authStore.state.isLoggedIn &&
                                    !!authStore.state.userData?.email
                                    " :rules="{ required: true, email: true }" inputPlaceholder="البريد الالكتروني" inputType="email" />
                            </div>
                            <form-input v-model:inputValue="form.title" inputId="title" :rules="{ required: true }"
                                inputPlaceholder="عنوان الرسالة" />
                            <form-input v-model:inputValue="form.message" :isTextArea="true" inputId="message"
                                :rules="{ required: true }" inputPlaceholder="نص الرسالة" />

                            <lazy-app-button :isLoading="addRequest.status.value === 'pending'" class="self-end mt-[20px]"
                                :label="'أرسل رسالة'" @click="handleSubmit(submit)" />
                        </vee-form>
                    </div>
                </lazy-vee-validate-provider>
            </div>
            <div class="cl-start">
                ...
                <div class="social-buttons">
                    <a :href="jsonData.snapchat" target="_blank">
                        <i class="fab fa-snapchat"></i>
                    </a>
                    <a :href="jsonData.youtube" target="_blank">
                        <i class="fab fa-youtube"></i>
                    </a>
                    <a :href="jsonData.tiktok" target="_blank">
                        <i class="fab fa-tiktok"></i>
                    </a>
                    <a :href="jsonData.instagram" target="_blank">
                        <i class="fab fa-instagram"></i>
                    </a>
                    <a :href="jsonData.twitter" target="_blank">
                        <svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 512 512" fill="#fff">
                            <path
                                d="M389.2 48h70.6L305.6 224.2 487 464H345L233.7 318.6 106.5 464H35.8L200.7 275.5 26.8 48H172.4L272.9 180.9 389.2 48zM364.4 421.8h39.1L151.1 88h-42L364.4 421.8z" />
                        </svg>
                    </a>
                    <a :href="jsonData.facebook" target="_blank">
                ...
                <div class="contacts">
                    <div class="c-group">
                        ...
                        <span>
                            <a :href="`mailto:${jsonData.email}`" target="_blank">
                                {{ jsonData.email }}
                            </a>
                        </span>
                    </div>
...
useHead({
    ...
    meta: [
        ...
        },
        {
            ...
            content: `${jsonData.value?.metaTitle ? jsonData.value.metaTitle : defaultMeta.title
                }`,
        },
        {
            property: 'twitter:description',
            content: `${jsonData.value?.metaDescription
...
<style lang="scss" scoped src="@/assets/scss/shared/web-content-page.scss"></style>
